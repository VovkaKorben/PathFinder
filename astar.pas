...
uses
    Sysutils, Classes, uConstants, System.Generics.Collections;
...
begin
    ...
    try
        try
            ...
            if not check_DB_error(sqlite3_prepare_v2(db, 'SELECT id FROM "items" WHERE allow_deposit=1', -1, stmt, nil)) then
                Exit;
            try
                allowDeposit.Clear;
                while sqlite3_step(stmt) = 100 do
                    allowDeposit.Add(sqlite3_column_int(stmt, 0));
                allowDeposit.Sort;
...
initialization
    allowDeposit := TList<Integer>.Create;
    FullDbPath := 'C:\la_db\new.db3';
